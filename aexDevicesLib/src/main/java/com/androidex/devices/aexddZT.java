...
public class aexddZT extends aexddPasswordKeypad {
    static
    {
        try {
    ...
    public static final String TAG = "aexddZT";

    public aexddZT(Context ctx) {
        super(ctx);
    }

    public aexddZT(Context ctx, JSONObject args) {
        super(ctx, args);
    }

    @Override
    public String getDeviceName() {
        return mContext.getString(R.string.DEVICE_PK_ZT);
    }

    @Override
    public boolean Open() {
        android.util.Log.d("aexddZT","mparams="+mParams.toString());
        String printerPort = mParams.optString(PORT_ADDRESS);
        if(mSerialFd > 0)
            Close();
        //String ret = kmyOpen(printerPort);
        String ret = ztOpen(printerPort);
        try {
    ...
    public String pkGetVersion()
    {
        ...
        */
        ...
        return ztGetVersion(3000*delayUint);
    }
    // jni相关函数
    public  native String  ztOpen(String arg);//传入printerPort
    public  native void ztClose();
    public  native int ztReset(int timeout);
   ...
   // public  native int kmySetSn(String sn,int timeout);
    public  native String ztGetVersion(int timeout);

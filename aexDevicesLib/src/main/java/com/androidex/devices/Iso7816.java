...
//		}
	...
	public final static class BerHouse {
		final ArrayList<Iso7816.BerTLV> tlvs = new ArrayList<Iso7816.BerTLV>();
...
//		}
	...
	public final static class StdTag {
		...
		public byte[] transceive(final byte[] cmd) throws IOException {
			try {
				byte[] rsp = null;
				final byte[] cmdCardInfo = {(byte) 0x00, (byte) 0xb0, (byte) 0x9f, (byte) 0x05, (byte) 0x80};
				byte c[] = cmd;
				 do {
					if (nfcTag!=null){
						Log.e("nfcTag.transceive(c):xxxx", "nfcTag.transceive异常1");
						if (!nfcTag.isConnected()){
							nfcTag.connect();
							nfcTag.setTimeout(5000);
						}
						Log.e("MaxTransceiveLength:xxxx", "MaxTransceiveLength: "+nfcTag.getMaxTransceiveLength());//获取卡片能接受的最大长度
						Log.e("nfcTag.getTag():xxxx", "nfcTag.getTag(): "+nfcTag.getTag());;
						byte[] r = nfcTag.transceive(cmdCardInfo);
						Log.e("nfcTag.transceive(c):xxxx", "nfcTag.transceive异常2");
						if (r == null)
							break;
						Log.e("nfcTag:xxxx", "rrrrr: "+r.toString());
						int N = r.length - 2;
						if (N < 0) {
							rsp = r;
							break;
						}
						Log.e("nfcTag.transceive(c):xxxx", "nfcTag.transceive异常3");
						if (r[N] == CH_STA_LE) {
							c[c.length - 1] = r[N + 1];
							continue;
						}
						Log.e("nfcTag.transceive(c):xxxx", "nfcTag.transceive异常4");
						if (rsp == null) {
							rsp = r;
						} else {
							int n = rsp.length;
						...
						Log.e("rsp:xxxx", "rsprsp1: "+rsp.toString());
						if (r[N] != CH_STA_MORE)
							break;
						...
						Log.e("nfcTag:xxxx", "nfcTag: is null Object");
					}
					Log.e("rsp:xxxx", "rsprsp2: "+rsp.toString());
				} while (true);
				Log.e("rsp:xxxx", "rsprsp3: "+rsp.toString());
				return rsp;
